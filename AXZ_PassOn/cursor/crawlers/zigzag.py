...
class ZigzagCrawler(BaseCrawler):
    """지그재그 사이트 전용: BaseCrawler 상속, site_config 완벽 전달."""
    ...
    async def _run_flow(self) -> List[Dict[str, Any]]:
        ...
        item_link_selector = self.site_config.get("item_link_selector", "a[href*='/product/'], a[href*='/shop/']")
        image_selector = self.site_config.get("image_selector", "img[src*='product']")
        ...
        await self.wait_for_manual_login(
            "지그재그 로그인 후 '주문 내역' 화면이 보이면 터미널에서 엔터를 눌러주세요: "
        )
        ...
        for block in order_blocks:
            ...
            for link in links[:50]:
                ...
                if not href or href in seen_hrefs:
                    continue
                if "/product/" not in href and "/shop/" not in href:
                    continue
                seen_hrefs.add(href)
                if not href.startswith("http"):
                    base_url = self.site_config.get("base_url", "https://www.zigzag.kr")
                    href = base_url.rstrip("/") + ("/" + href.lstrip("/") if not href.startswith("/") else href)
                title = await link.get_attribute("title") or (await link.inner_text()).strip() or "지그재그 상품"
                title = re.sub(r'[\\/*?:"<>|]', "_", title)[:100]
                print(f"[SCRAPE] 상품명 추출 성공: {title}")
                results.append({"url": href, "title": title, "site": "zigzag"})
        ...
        meta_path = os.path.join(self.temp_dir, "zigzag_orders.json")
    ...
    async def _save_item_images(
        ...
        try:
            ...
            for img in imgs[:15]:
                ...
                if src and ("product" in src or "zigzag" in src or "http" in src):
            ...
            if not urls:
                urls = await page.evaluate(
                    """() => Array.from(document.querySelectorAll('img'))
                        .map(i => i.src).filter(s => s && (s.includes('product') || s.includes('zigzag')))"""
                )
            ...
            for j, img_url in enumerate(urls[:10]):
                try:
                    ...
                    filename = f"zigzag_{index}_{j}_{safe_title}.{ext}"
